-- ═══════════════════════════════════════════════════════════
-- Security fix 9
--   1. Lock auto_guess_expired_mysteries() so only cron / service
--      role / admins can call it. Random clients can't trigger DB load.
--   2. Drop unused get_next_mystery_start (orphan function in prod).
--   3. Add season-membership/admin gate to get_season_mysteries and
--      get_next_mystery_info — previously they returned mystery data
--      (images, titles for revealed, non-sensitive answer fields) for
--      ANY season ID, even from seasons the caller wasn't a member of.
-- ═══════════════════════════════════════════════════════════

-- ── 1. Lock auto_guess_expired_mysteries ──
-- auth.uid() returns the caller's UUID for authenticated clients and
-- NULL for postgres/service_role/cron contexts (no JWT). So the check
-- only blocks regular logged-in users; cron and edge functions still work.
...
begin
    -- Access control: block authenticated non-admins. Cron/service role
    -- have NULL auth.uid() and are unaffected.
...
-- ── 2. Drop unused orphan function ──
DROP FUNCTION IF EXISTS public.get_next_mystery_start(uuid);

-- ── 3. Gate get_season_mysteries on membership (or admin) ──
...
-- ── 4. Gate get_next_mystery_info on membership (or admin) ──
