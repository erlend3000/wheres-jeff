...
export function computeSchedule(mysteries, config) {
    ...
    if (!starts_at || !frequency_value || !gap_value) return mysteries;
    ...
    const durationMs = freqMs - gapMs;
    if (durationMs <= 0) return mysteries;
    ...
    for (const m of sorted) {
        ...
        m.reveals_at = new Date(pubMs + durationMs).toISOString();
        prevRevealsAt = pubMs + durationMs;
    }
    return sorted;
}
