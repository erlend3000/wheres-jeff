...
export async function removeMysteryInstance(instanceId) {
    ...
    const { error } = await supabase
        ...
        .eq('id', instanceId);
    if (error) throw error;
    if (row?.image_url) await removeStorageImage(row.image_url);
}
...
export async function deleteMysteryTemplate(templateId) {
    ...
    if (url) await removeStorageImage(url);
}
...
function removeStorageImage(publicUrl) {
    try {
        const path = publicUrl.split('/mystery-images/').pop()?.split('?')[0];
        if (path) return supabase.storage.from('mystery-images').remove([path]);
