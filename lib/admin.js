...
// Listen for any change to the seasons table so the admin games list can
// refresh when e.g. a player triggers checkSeasonAutoEnd from the player view.
export function subscribeSeasons(onChange) {
...
export async function getSeasonMembers(seasonId) {
    const { data, error } = await supabase.rpc('admin_get_season_members', { p_season_id: seasonId });
    if (error) throw error;
    return (data ?? []).map(r => ({
        user_id: r.user_id,
        profiles: { id: r.profile_id, name: r.profile_name, email: r.profile_email, role: r.profile_role }
    }));
}
...
export async function removeSeasonMemberGuesses(seasonId, userId) {
    ...
    const { error } = await supabase
        .from('guesses')
        .delete()
        .in('mystery_id', mysteries.map(m => m.id))
        .eq('user_id', userId);
    if (error) throw error;
}
...
export async function getSeasonScores(seasonId) {
    const allMysteries = await getMysteriesBySeason(seasonId);
    const now = new Date();
    const mysteries = allMysteries.filter(m => m.reveals_at && new Date(m.reveals_at) <= now);
    if (!mysteries.length) return {};
    ...
    const karmaByMystery = {};
    for (const [uid, userGuesses] of Object.entries(guessesByUser)) {
        if (scores[uid]) {
            const result = calculateKarma(userGuesses, mysteriesMap, allGuessesByMystery, cumulativeScores);
            scores[uid].karma = result.total;
            for (const [mid, delta] of Object.entries(result.byMystery)) {
                if (!karmaByMystery[mid]) karmaByMystery[mid] = {};
                karmaByMystery[mid][uid] = delta;
            }
        }
    }
    return { scores, karmaByMystery };
}
...
function calculateKarma(userGuesses, mysteriesMap, allGuessesByMystery, cumulativeScores) {
    ...
    for (const g of sorted) {
        ...
        byMystery[g.mystery_id] = karma - karmaBefore;
    }
    return { total: karma, byMystery };
}

export async function getGuessesForMystery(mysteryId) {
    const { data, error } = await supabase
        .from('guesses')
        .select('user_id, score, distance_m, submitted_at, guess, profiles(name)')
...
export async function removeMysteryInstance(instanceId) {
    ...
    const { error } = await supabase
        ...
        .eq('id', instanceId);
    if (error) throw error;
    if (row?.image_url) await removeStorageImageIfOrphan(row.image_url);
}
...
export async function deleteMysteryTemplate(templateId) {
    ...
    if (error) throw error;
    const url = rows?.find(r => r.image_url)?.image_url;
    if (url) await removeStorageImageIfOrphan(url);
}
...
async function removeStorageImageIfOrphan(publicUrl) {
    try {
        ...
        if (count > 0) return;
        const path = publicUrl.split('/mystery-images/').pop()?.split('?')[0];
        if (path) await supabase.storage.from('mystery-images').remove([path]);
...
export async function getUsers() {
    const { data: profiles, error: pErr } = await supabase.rpc('admin_get_profiles');
    if (pErr) throw pErr;
    ...
    return profiles.map(p => ({
        ...
        jeff_karma: calculateKarma(guessesByUser[p.id] ?? [], mysteriesMap, allGuessesByMystery, cumulativeScores).total
    }));
}
