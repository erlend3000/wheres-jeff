...
export const RANGE_UNIT_TO_YEARS = { minute: 1 / 525960, hour: 1 / 8766, day: 1 / 365.25, month: 1 / 12, year: 1 };

/** True when the correct answer is an explicit time window (new or legacy ± range). */
export function isAnswerRange(answer) {
    if (!answer) return false;
    if (answer.answer_start && answer.answer_end) return true;
    return !!(answer.range > 0 && answer.range_unit);
}

/** Point used for pin, title, and guess-period defaults (top-level year on range answers). */
export function getAnswerDisplayTime(answer) {
    ...
    if (answer.year != null) {
        return {
            ...
            bc: answer.bc,
            detail_level: answer.detail_level,
        };
    }
    if (answer.answer_start && answer.answer_end) {
        const mid = (timeToDecimal(answer.answer_start) + timeToDecimal(answer.answer_end)) / 2;
        return { ...decimalToTime(mid), detail_level: answer.detail_level || 'year' };
    }
    return null;
}

/** Decimal year at the display point (or centre fallback for range without display year). */
export function getAnswerCenterDecimal(answer) {
    const t = getAnswerDisplayTime(answer);
    return t ? timeToDecimal(t) : 0;
}

/** Human-readable width of an explicit answer range. */
export function formatAnswerRangeSpan(answer) {
    if (!answer?.answer_start || !answer?.answer_end) return '';
    const span = Math.abs(timeToDecimal(answer.answer_end) - timeToDecimal(answer.answer_start));
    if (span === 0) return '0';
    return formatTimeDifference(span);
}

/**
 * Effective distance from guess to correct answer.
 * - Explicit range (answer_start/answer_end): 0 if inside, else distance to nearest edge.
 * - Legacy ± range: symmetric tolerance around the point.
 * - Point only: plain year difference.
 */
export function effectiveTimeDiff(guess, answer) {
    const dl = answer?.detail_level || 'minute';
    const gDec = timeToDecimal(truncateToDetail(guess, dl));

    if (answer?.answer_start && answer?.answer_end) {
        let lo = timeToDecimal(truncateToDetail(answer.answer_start, dl));
        let hi = timeToDecimal(truncateToDetail(answer.answer_end, dl));
        if (lo > hi) { const t = lo; lo = hi; hi = t; }
        if (gDec >= lo && gDec <= hi) return 0;
        return Math.min(Math.abs(gDec - lo), Math.abs(gDec - hi));
    }

    let diff = timeYearsDiff(guess, answer, dl);
...
export function formatTimeDifference(yearsDiff) {
    ...
    if (a < 1 / 365.25) {
        const hrs = a * 8766;
        if (hrs < 1) { const m = Math.round(hrs * 60); return `${m} ${m === 1 ? 'minute' : 'minutes'}`; }
        const v = parseFloat(hrs.toFixed(1));
        return `${v} ${v === 1 ? 'hour' : 'hours'}`;
    }
    if (a < 1 / 12) { const d = Math.round(a * 365.25); return `${formatNumber(d)} ${d === 1 ? 'day' : 'days'}`; }
    if (a < 1) { const mo = parseFloat((a * 12).toFixed(1)); return `${mo} ${mo === 1 ? 'month' : 'months'}`; }
    if (a < 10000) { const y = parseFloat(a.toFixed(1)); return `${formatNumber(y)} ${y === 1 ? 'year' : 'years'}`; }
    if (a < 1_000_000) { const y = Math.round(a); return `${formatNumber(y)} years`; }
...
export function formatTimeDifferenceShort(yearsDiff) {
    ...
    if (a < 1 / 365.25) {
        const hrs = a * 8766;
        if (hrs < 1) { const m = Math.round(hrs * 60); return `${formatNumber(m)} min`; }
        const v = parseFloat(hrs.toFixed(1));
        return `${formatNumber(v)} ${v === 1 ? 'hr' : 'hrs'}`;
    }
    if (a < 1 / 12) { const d = Math.round(a * 365.25); return `${formatNumber(d)} d`; }
    if (a < 1) { const mo = parseFloat((a * 12).toFixed(1)); return `${formatNumber(mo)} mo`; }
...
export function formatTimeTitle(answer) {
    ...
    if (answer.answer_start && answer.answer_end) {
        const display = getAnswerDisplayTime(answer);
        if (display) return formatTimeTitle(display);
        return 'Unknown';
    }
    if (answer.year == null) return 'Unknown';
...
export async function submitTimeGuess(mysteryId, guessTime, { autoGuess = false, autoSubmit = false } = {}) {
    const { data, error } = await supabase.rpc('submit_time_guess', {
        p_mystery_id: mysteryId,
        p_guess: guessTime,
        p_auto_guess: autoGuess,
        p_auto_submit: autoSubmit
    });
    if (error) throw error;
    if (data?.user_id) broadcastSubmission(mysteryId, data.user_id, data.submitted_at).catch(() => {});
    return data;
}
...
// English possessive: "Alice's" but "Nilas'" when the name already ends in s.
// Pass { upper: true } to get an uppercase 'S (e.g. for all-caps headings).
export function possessive(name, { upper = false } = {}) {
    if (!name) return '';
    if (/s$/i.test(name)) return `${name}'`;
    return upper ? `${name}'S` : `${name}'s`;
}
...
export async function reverseGeocode(lat, lng) {
    let normLng = ((lng % 360) + 540) % 360 - 180;
    const key = geocodeCacheKey(lat, normLng);
    ...
    try {
        const res = await fetch(
            `https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${normLng}&format=json&addressdetails=1&zoom=18&accept-language=en`,
...
export function formatTimeAgo(submittedAt, publishedAt) {
    ...
    if (hours < 24) { const v = parseFloat(hours.toFixed(1)); return `${v} ${v === 1 ? 'hr' : 'hrs'}`; }
    const days = hours / 24;
...
export async function getSeasonWinners(seasonId, mode = 'best70', { testerIds, currentUserId } = {}) {
    const board = await getSeasonLeaderboard(seasonId, mode, { testerIds, currentUserId });
...
export async function getSeasonMysteries(seasonId) {
    const { data, error } = await supabase.rpc('get_season_mysteries', { p_season_id: seasonId });
    if (error) throw error;
    return data ?? [];
}
...
let _testerIdsCache = null;

export async function getSeasonTesterIds() {
    if (_testerIdsCache) return _testerIdsCache;
    const { data } = await supabase.rpc('get_tester_ids');
    _testerIdsCache = new Set((data ?? []).map(r => r.id));
    return _testerIdsCache;
}

export function resetTesterCache() {
    _testerIdsCache = null;
}

function filterTesters(items, testerIds, currentUserId) {
    if (!testerIds || !testerIds.size) return items;
    return items.filter(g => !testerIds.has(g.user_id) || g.user_id === currentUserId);
}

// ── Guesses ──

export async function getGuesses(mysteryId, { testerIds, currentUserId } = {}) {
    const { data, error } = await supabase
        .from('guesses')
        ...
        .order('submitted_at');
    if (error) throw error;
    return filterTesters(data ?? [], testerIds, currentUserId);
}

export async function getSubmissions(mysteryId, { testerIds, currentUserId } = {}) {
    const { data, error } = await supabase.rpc('get_mystery_submissions', { p_mystery_id: mysteryId });
    if (error) throw error;
    const rows = (data ?? []).map(r => ({ user_id: r.user_id, submitted_at: r.submitted_at, profiles: { name: r.name } }));
    return filterTesters(rows, testerIds, currentUserId);
}
...
export async function submitGuess(mysteryId, lat, lng, { autoGuess = false, autoSubmit = false } = {}) {
    const location = await reverseGeocode(lat, lng);
    const { data, error } = await supabase.rpc('submit_location_guess', {
        p_mystery_id: mysteryId,
        p_lat: lat,
        p_lng: lng,
        p_location: location,
...
// Realtime "X submitted" notifications use a broadcast channel rather than
// postgres_changes on `guesses`, because RLS hides other players' guesses on
// active (non-revealed) mysteries — which would filter out the realtime push.
// The submitting client emits a broadcast after its RPC succeeds; receivers
// fetch the profile name and update their UI. Only metadata is broadcast,
// never the actual guess content.
//
// We keep one module-level channel per mystery so that submitGuess /
// submitTimeGuess can send on the same already-subscribed channel that
// subscribeGuesses opened — avoiding the race of opening/closing a fresh
// channel for each broadcast.
const submissionChannels = new Map();

function getOrCreateSubmissionChannel(mysteryId) {
    let entry = submissionChannels.get(mysteryId);
    if (entry) return entry;
    const channel = supabase.channel(`submissions:${mysteryId}`, { config: { broadcast: { self: false } } });
    entry = { channel, listeners: new Set(), subscribed: false };
    submissionChannels.set(mysteryId, entry);
    channel.on('broadcast', { event: 'submission' }, async ({ payload }) => {
        const guess = payload || {};
        if (!guess.user_id) return;
        for (const listener of entry.listeners) {
            try { await listener(guess); } catch {}
        }
    });
...
export function subscribeGuesses(mysteryId, onInsert, { testerIds, currentUserId } = {}) {
    const entry = getOrCreateSubmissionChannel(mysteryId);
    const wrapped = async (guess) => {
        if (testerIds && testerIds.has(guess.user_id) && guess.user_id !== currentUserId) return;
    ...
    return {
        unsubscribe() {
            ...
            if (entry.listeners.size === 0) {
                ...
                submissionChannels.delete(mysteryId);
            }
        }
    };
}

export async function broadcastSubmission(mysteryId, userId, submittedAt) {
    const entry = getOrCreateSubmissionChannel(mysteryId);
    if (!entry.subscribed) {
        await new Promise((resolve) => {
            const cleanup = () => { clearTimeout(timer); clearInterval(check); };
            const timer = setTimeout(() => { cleanup(); resolve(); }, 3000);
            const check = setInterval(() => {
                if (entry.subscribed) { cleanup(); resolve(); }
            }, 50);
        });
    }
    try {
        await entry.channel.send({
            type: 'broadcast',
            event: 'submission',
            payload: { user_id: userId, submitted_at: submittedAt }
        });
    } catch {}
}
...
function topNCount(N) {
    if (N <= 1) return N;
    if (N === 2) return 1;
    return Math.max(1, Math.round(0.7 * N));
}

function selectGuessesForMode(allGuesses, ids, mode) {
    ...
    if (mode === 'best70') {
        ...
        if (K < N) {
            ...
            for (const g of allGuesses) {
                if (!inIds.has(g.mystery_id)) continue;
                (byUser[g.user_id] ??= []).push(g);
            }
            const out = [];
            for (const ug of Object.values(byUser)) {
                const sorted = [...ug].sort((a, b) => b.score - a.score);
                out.push(...sorted.slice(0, K));
            }
            return out;
        }
    }
    return allGuesses.filter(g => inIds.has(g.mystery_id));
}

export async function getSeasonLeaderboard(seasonId, mode = 'best70', { testerIds, currentUserId } = {}) {
    const { data: mysteries, error: mErr } = await supabase
        .from('mysteries')
        .select('id, type, answer, published_at, reveals_at')
        .eq('season_id', seasonId)
        .not('published_at', 'is', null)
    ...
    const allRevealedIds = revealed.map(m => m.id);
    const baseIds = mode === 'latest' ? allRevealedIds.slice(0, 1) : allRevealedIds;
    const prevIds = mode === 'latest' ? [] : baseIds.filter(id => id !== baseIds[0]);

    const { data: guesses, error: gErr } = await supabase
        .from('guesses')
        .select('user_id, score, distance_m, submitted_at, mystery_id, guess, profiles(name)')
        .in('mystery_id', allRevealedIds);
    if (gErr) throw gErr;
    const visibleGuesses = filterTesters(guesses ?? [], testerIds, currentUserId);

    const curGuesses = selectGuessesForMode(visibleGuesses, baseIds, mode);
    const prevGuesses = prevIds.length ? selectGuessesForMode(visibleGuesses, prevIds, mode) : [];

    function buildBoard(filteredGuesses) {
        const map = {};
        const bestByMystery = {};
    ...
    const current = buildBoard(curGuesses);
    const previous = prevIds.length ? buildBoard(prevGuesses) : [];

    const allGuessesByMystery = {};
    for (const g of visibleGuesses) {
        (allGuessesByMystery[g.mystery_id] ??= []).push(g);
    }

    function buildKarmaContext(filteredGuesses, ids) {
        ...
        const board = buildBoard(filteredGuesses);
        const scores = {};
    ...
    const curCtx = buildKarmaContext(curGuesses, baseIds);
    computeKarma(current, curCtx);

    const prevCtx = prevIds.length ? buildKarmaContext(prevGuesses, prevIds) : null;
    ...
    if (last4.length >= 4) {
        ...
        const prev2Ids = baseIds.filter(id => id !== baseIds[0] && id !== baseIds[1]);
        const prev3Ids = baseIds.filter(id => id !== baseIds[0] && id !== baseIds[1] && id !== baseIds[2]);
        const prev2Guesses = prev2Ids.length ? selectGuessesForMode(visibleGuesses, prev2Ids, mode) : [];
        const prev3Guesses = prev3Ids.length ? selectGuessesForMode(visibleGuesses, prev3Ids, mode) : [];
        const prev2Ctx = prev2Ids.length ? buildKarmaContext(prev2Guesses, prev2Ids) : null;
        const prev3Ctx = prev3Ids.length ? buildKarmaContext(prev3Guesses, prev3Ids) : null;
...
export async function getMysteryLeaderboard(mysteryId, publishedAt, revealsAt, { testerIds, currentUserId } = {}) {
    const guesses = await getGuesses(mysteryId, { testerIds, currentUserId });
    return guesses
        .map(g => {
            const isAuto = g.guess?.auto_guess === true;
