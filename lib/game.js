...
function calculateJeffKarma(userGuesses, mysteriesMap) {
    ...
    for (const g of sorted) {
        ...
        if (isAuto) {
            ...
            // Klikk-og-ferdig: guessed within 5 seconds
            if (elapsedSec < 5) {
        ...
        if (scores.length >= 4) {
            ...
            if (current > avg3) {
                // Forbedring: scored above average of last 3
                karma += 1000 + Math.round(current - avg3);
            } else if (current < Math.min(...recent3)) {
                // Fritt fall: scored below all of last 3
...
export async function getMysteryLeaderboard(mysteryId, publishedAt) {
    ...
    return guesses
        .map(g => ({
            ...
            time: formatTimeAgo(g.submitted_at, publishedAt),
            guess: g.guess,
