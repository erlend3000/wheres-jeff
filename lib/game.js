...
export async function getSeasonWinners(seasonId, mode = 'best70') {
    const board = await getSeasonLeaderboard(seasonId, mode);
...
export async function getSeasonLeaderboard(seasonId, mode = 'best70') {
    ...
    const curGuesses = selectGuessesForMode(guesses ?? [], baseIds, mode);
    const prevGuesses = prevIds.length ? selectGuessesForMode(guesses ?? [], prevIds, mode) : [];
    ...
    if (last4.length >= 4) {
        ...
        const prev2Guesses = prev2Ids.length ? selectGuessesForMode(guesses ?? [], prev2Ids, mode) : [];
        const prev3Guesses = prev3Ids.length ? selectGuessesForMode(guesses ?? [], prev3Ids, mode) : [];
