...
export function subscribeGuesses(mysteryId, onInsert, { testerIds, currentUserId } = {}) {
    const channel = supabase
        .channel(`submissions:${mysteryId}`, { config: { broadcast: { self: false } } })
...
export async function broadcastSubmission(mysteryId, userId, submittedAt) {
    const ch = supabase.channel(`submissions:${mysteryId}`);
    try {
        ...
        await ch.send({ type: 'broadcast', event: 'submission', payload: { user_id: userId, submitted_at: submittedAt } });
    } finally {
        ch.unsubscribe();
    }
}
