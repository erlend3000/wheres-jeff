...
/** Decimal year at the centre of a point or range answer (for guess-period defaults). */
export function getAnswerCenterDecimal(answer) {
    if (!answer) return 0;
    if (answer.answer_start && answer.answer_end) {
        return (timeToDecimal(answer.answer_start) + timeToDecimal(answer.answer_end)) / 2;
    }
    return timeToDecimal(answer);
}
...
export function formatTimeTitle(answer) {
    ...
    const dl = answer.detail_level || 'year';
    if (answer.answer_start && answer.answer_end) {
        const a = { ...answer.answer_start, detail_level: dl };
        const b = { ...answer.answer_end, detail_level: dl };
        return `${formatTimeTitle(a)} – ${formatTimeTitle(b)}`;
    }
