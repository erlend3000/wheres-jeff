...
export async function getSeasonWinners(seasonId) {
    ...
    const winsWinner = [...board].sort((a, b) => b.wins - a.wins || b.total_score - a.total_score)[0];
...
export async function getSeasonLeaderboard(seasonId) {
    ...
    for (const entry of current) cumulativeScores[entry.user_id] = entry.total_score;

    current.forEach(entry => {
        entry.jeff_karma = calculateJeffKarma(guessesByUser[entry.user_id] ?? [], mysteriesMap, allGuessesByMystery, cumulativeScores);
    });

    const prevGuessesByUser = {};
    const prevAllGuessesByMystery = {};
    const prevRevealedMap = {};
    for (const m of revealed) {
        if (prevIds.includes(m.id)) prevRevealedMap[m.id] = m;
    }
    for (const g of (guesses ?? [])) {
        if (!prevIds.includes(g.mystery_id)) continue;
        if (!prevGuessesByUser[g.user_id]) prevGuessesByUser[g.user_id] = [];
        prevGuessesByUser[g.user_id].push(g);
        if (!prevAllGuessesByMystery[g.mystery_id]) prevAllGuessesByMystery[g.mystery_id] = [];
        prevAllGuessesByMystery[g.mystery_id].push(g);
    }
    const prevCumulativeScores = {};
    for (const entry of previous) prevCumulativeScores[entry.user_id] = entry.total_score;
    previous.forEach(entry => {
        entry.jeff_karma = calculateJeffKarma(prevGuessesByUser[entry.user_id] ?? [], prevRevealedMap, prevAllGuessesByMystery, prevCumulativeScores);
    });
