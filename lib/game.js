...
let _testerIdsCacheSeason = null;

export async function getSeasonTesterIds(seasonId) {
    if (_testerIdsCacheSeason === seasonId && _testerIdsCache) return _testerIdsCache;
    const { data } = await supabase
        .from('season_members')
        .select('user_id')
        .eq('season_id', seasonId)
        .eq('role', 'tester');
    _testerIdsCache = new Set((data ?? []).map(r => r.user_id));
    _testerIdsCacheSeason = seasonId;
    return _testerIdsCache;
}

export function resetTesterCache() {
    _testerIdsCache = null;
    _testerIdsCacheSeason = null;
}
